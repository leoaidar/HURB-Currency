...
describe('CurrencyControllerTest', () => {
  ...
  describe('getCurrency', () => {
    it('should retrieve a currency by ID', async () => {
      ...
      expect(await controller.getCurrency({ id: '1' } as IdParamDto)).toBe(
        currency,
      );
    });
    ...
    it('should throw an error if currency not found', async () => {
      ...
      await expect(
        controller.getCurrency({ id: '1' } as IdParamDto),
  ...
  describe('updateCurrency', () => {
    it('should update a currency', async () => {
      ...
      expect(await controller.updateCurrency(code, dto)).toEqual(
        updatedCurrency,
      );
    });
  });
  ...
  describe('getAllCurrencies', () => {
    it('should retrieve all currencies', async () => {
      ...
      jest
        .spyOn(service, 'getAllCurrencies')
        .mockImplementation(async () => currencies);
  ...
  describe('updateCurrencyRates', () => {
    it('should update currency exchange rates', async () => {
      ...
      jest
        .spyOn(service, 'updateCurrencyRates')
        .mockImplementation(async () => message);

      expect(await controller.updateExchangeRates(currencySymbols)).toEqual({
        message,
      });
    });
  ...
  describe('deleteCurrency', () => {
    it('should delete a currency by ID', async () => {
      ...
      expect(await controller.deleteCurrency({ id } as IdParamDto)).toEqual({
        message: 'Currency deleted successfully',
      });
    });
    ...
    it('should throw an error if currency not found on delete', async () => {
      ...
      await expect(
        controller.deleteCurrency({ id } as IdParamDto),
