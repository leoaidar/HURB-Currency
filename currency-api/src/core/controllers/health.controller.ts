...
export class HealthController {
  ...
  @ApiResponse({
    status: 200,
    description: 'check: currency Quote API, Database and this Microservice are UP. ',
  })
  @ApiResponse({
    status: 500,
    description: 'check: currency Quote API, Database and this Microservice possibly DOWN ',
  })
  async checkHealth() {
    const healthStatus = {
  ...
  private async checkExternalAPI(): Promise<string> {
    try {
      ...
      if (error instanceof Error) {
        ...
        this.logger.error('Falha ao verificar a API externa', 'Erro desconhecido');
      }
      return 'DOWN';
    }
  }
  ...
  private async checkDatabase(): Promise<string> {
    try {
      ...
      this.logger.log(`Moeda que foi deletada: ${JSON.stringify(foundCurrency)}`);
  
      return foundCurrency ? 'UP' : 'DOWN';
    } catch (error: unknown) {
      this.logger.error('Falha ao verificar o banco de dados', error instanceof Error ? error.message : 'Erro desconhecido');
      return 'DOWN';
    } finally {
  ...
  private async checkInternalAPI(): Promise<string> {
    try {
      this.logger.log('Verificando o endpoint interno de microserviço...');
      ...
      this.logger.error('Falha ao verificar o microserviço interno', error);
      return 'DOWN';
    }
  }
