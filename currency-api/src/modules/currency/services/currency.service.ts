...
export class CurrencyService {
  ...
  async updateCurrencyRate(
    ...
    this.logger.log(
      `Atualizando a taxa da moeda... ID: ${id}, taxa: ${newRate}`,
    );
    return this.currencyModel
      .findOneAndUpdate({ id: id }, { rate: newRate }, { new: true })
      .exec();
  }
  ...
  async updateCurrency(
    ...
    this.logger.log(
      `Moeda atualizada com sucesso: ${JSON.stringify(updatedCurrency)}`,
    );
  ...
  async deleteCurrency(
    ...
    try {
      ...
      return {
        deleted: false,
        message: `Erro ao deletar a moeda com ID: ${id}`,
      };
    }
  }
  ...
  async updateCurrencyRates(currencySymbols?: string[]): Promise<string> {
    ...
    const currencyRates = await this.exchangeRateService.getExchangeRate(
      'USD',
      symbols,
    );
    ...
    await Promise.all(
      currencies.map(async (currency) => {
        ...
        if (newRate) {
          ...
          this.logger.log(
            `Updated ${currency.code.toUpperCase()} with rate ${newRate}`,
          );
        }
      }),
    );
  ...
  async convertCurrency(
    ...
    if (isNaN(parsedAmount) || !/^[\d]+(\.[\d]{1,2})?$/.test(amount)) {
      this.logger.log(
        'Valor inválido fornecido. Assegure que é um número com até 2 casas decimais.',
      );
      throw new CurrencyInvalidAmountException();
    }

    // Chama a API externa de integração de câmbio
    const currenciesRates = await this.exchangeRateService.getExchangeRate(
      from,
      [to],
    );
    this.logger.log(
      `Taxas de câmbio obtidas: ${JSON.stringify(currenciesRates)}`,
    );
    ...
    let rateFrom =
      from === 'USD' ? 1 : currenciesRates.rates[from.toLowerCase()];

    // Se as taxas não foram encontradas na API externa, busca no banco de dados local
    if (!rateTo) {
      this.logger.log(
        'Taxa de câmbio não encontrada na API externa, buscando no banco de dados local.',
      );
      const localCurrencies = await this.currencyModel
        .find({ code: { $in: [from, to] } })
        .exec();
      const localCurrencyMap = localCurrencies.reduce(
        (currenciesRates, currentElement) => {
          currenciesRates[currentElement.code.toLowerCase()] =
            currentElement.rate;
    ...
    const amountInternalCalculated = await this.convertCurrencyByRateUSD(
      parsedAmount,
      rateFrom,
      rateTo,
    );
  ...
  async convertCurrencyByRateUSD(
    amount: number,
    fromRate: number,
    toRate: number,
  ): Promise<number> {
    ...
    this.logger.log(
      `amount: number, fromRate: number, toRate: number: ${amount}, ${fromRate}, ${toRate}`,
    );
